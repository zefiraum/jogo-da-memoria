...
export function Login() {
  ...
  const [cssInputs, setCssInputs] = useState({
    cssUsername: false,
    cssPassword: false,
  });
  ...
  const handleCssInputs = (e) => {
    if (e.target.getAttribute("name") == "username") {
      setCssInputs({
        cssUsername: !cssInputs.cssUsername,
        cssPassword: cssInputs.cssPassword,
      });
    } else if (e.target.getAttribute("name") == "password") {
      setCssInputs({
        cssUsername: cssInputs.cssUsername,
      ...
      <section className="login">
        <div className="wrapper">
          ...
          <label className="input__label">
            <span
              className={`${cssInputs.cssUsername ? "span-active" : ""} ${
                login.username.length > 0 ? "span-active" : ""
              }`}
            >
              nome de usuário
            </span>
            <input
              ...
              name="username"
              className="input"
              onFocus={(e) => handleCssInputs(e)}
              onBlur={(e) => handleCssInputs(e)}
              onChange={(e) => handleInputs(e)}
            />
          </label>
          <label className="input__label">
            <span
              className={`${cssInputs.cssPassword ? "span-active" : ""} ${
                login.password.length > 0 ? "span-active" : ""
              }`}
            >
              senha
            </span>
